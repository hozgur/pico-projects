...
class R_Encoder:
    ...
    def Setup_Irqs(self):
        self.Enc_Pin_A = machine.Pin(self.A_Pin, machine.Pin.IN, machine.Pin.PULL_UP)
        self.Enc_Pin_A.irq(trigger=machine.Pin.IRQ_RISING | machine.Pin.IRQ_FALLING, handler=self.Enc_Handler_A)
        self.Enc_Pin_B = machine.Pin(self.B_Pin, machine.Pin.IN)
...
button_Enc_Val_Rst = machine.Pin(21, machine.Pin.IN, machine.Pin.PULL_UP)
Rotary_Enc = R_Encoder(15,14,7)

while True:
    time.sleep(0.1)
    #print(Rotary_Enc.Enc_Counter)
    if button_Enc_Val_Rst.value() == 0:
        print("Button Pressed")
        degree = Rotary_Enc.Enc_Counter * 360 / 20000
        print("Angle: ", degree)
        Rotary_Enc.show()
