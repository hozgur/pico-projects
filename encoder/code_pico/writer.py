...
class CWriter(Writer):
    ...
    def __init__(self, device, font, fgcolor=None, bgcolor=None, verbose=True):
        ...
        self.palbuf = bytearray(2 * 2) # 2 colors for 2 pixels
        self.palette = framebuf.FrameBuffer(self.palbuf, 2, 1, framebuf.RGB565)
    ...
    def _printchar(self, char, invert=False, recurse=False):
        ...
        fbc = framebuf.FrameBuffer(buf, self.clip_width, self.char_height, self.map)
        fg = self.fgcolor if not invert else self.bgcolor
        bg = self.bgcolor if not invert else self.fgcolor
